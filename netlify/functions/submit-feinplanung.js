...
<head>
  ...
  <title>Feinplanung übermittelt | Zukunftssolar</title>
  <style>
    body{
      margin:0;
      min-height:100vh;
      ...
      color:#f3fff9;
      background:
        radial-gradient(circle at top left, rgba(27,180,110,.28), transparent 35%),
        radial-gradient(circle at bottom right, rgba(27,180,110,.12), transparent 32%),
        linear-gradient(135deg,#05110d,#091b15 45%,#030807);
      padding:22px;
    }
    .card{
      width:100%;
      max-width:620px;
      background:rgba(15,36,28,.9);
    ...
    .logo{
      ...
      height:62px;
      border-radius:18px;
      background:linear-gradient(135deg,#1bb46e,#d7ffe7);
      color:#06140f;
      display:grid;
      place-items:center;
      font-weight:900;
    ...
    p{color:#cce6dc;line-height:1.6;font-size:16px}
    .btn{
      display:inline-block;
      margin-top:18px;
      padding:14px 20px;
      border-radius:16px;
      background:linear-gradient(135deg,#1bb46e,#baffd2);
      color:#06140f;
      font-weight:900;
      text-decoration:none;
    }
    .small{font-size:13px;color:#a8c7ba;margin-top:18px}
  </style>
</head>
<body>
  <div class="card">
    <div class="logo">ZS</div>
    <h1>Feinplanung erfolgreich übermittelt</h1>
    <p>Die Daten inklusive Bilder wurden an Jotform gesendet und per E-Mail an Zukunftssolar übermittelt.</p>
    <a class="btn" href="/">Neue Feinplanung erfassen</a>
    <div class="small">Du kannst diesen Tab schließen oder eine neue Feinplanung starten.</div>
  </div>
</body>
</html>`;
}
...
exports.handler = async (event) => {
  try{
    ...
    return { statusCode: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' }, body: successPage() };
  }catch(err){
